# Damp Concrete Web — associations.dampconcrete.com + dampconcrete.com

## Context

Read `docs/ARCHITECTURE.md` and `docs/CLAUDE.md` before starting.

This prompt builds `services/damp-concrete-web` — a Next.js app serving two subdomains:
- `dampconcrete.com` — simple holding page
- `associations.dampconcrete.com` — Associations marketing page

Both are served from one Railway service using subdomain routing via middleware. Read every file before touching it. No scope creep.

---

## Task A — Scaffold Next.js app

```bash
cd services/damp-concrete-web
npx create-next-app@latest . --app --js --no-tailwind --no-eslint --no-src-dir
```
...
## Task B — Subdomain routing middleware

Create `middleware.js` at the root of `services/damp-concrete-web`:
...
## Task C — Global styles

Create `app/globals.css`:

```css
@import url('https://fonts.googleapis.com/css2?family=Lora:ital,wght@0,400;1,400&family=Poppins:wght@300;400&display=swap');
...
## Task D — dampconcrete.com holding page
  ...
  return (
    <main style={{
      ...
      <nav style={{ display: 'flex', gap: '32px', flexWrap: 'wrap', justifyContent: 'center' }}>
        <a href="https://associations.dampconcrete.com" style={{
          fontFamily: "'Poppins', sans-serif",
          fontSize: '14px',
        ...
        <a href="https://terryheath.com" style={{
          fontFamily: "'Poppins', sans-serif",
          fontSize: '14px',
          letterSpacing: '0.06em',
          color: 'var(--text-faint)',
...
Copy the Damp Concrete logo to `public/damp-concrete-logo.png`.

---

## Task E — Associations marketing page

Create `app/associations/page.js`:
...
## Task F — Environment variables

Create `services/damp-concrete-web/.env.local`:

```
NEXT_PUBLIC_CF_ANALYTICS_TOKEN=your-cloudflare-analytics-token
```

Add to Railway env vars when deploying.

---

## Task G — next.config.js

Create `services/damp-concrete-web/next.config.js`:
...
Add Railway start command: `npm run start` with `npm run build` as build command.

---

## Cloudflare DNS (after Railway deployment)

In Cloudflare for `dampconcrete.com`:

- `dampconcrete.com` → CNAME to Railway domain, proxy on
- `associations.dampconcrete.com` → CNAME to same Railway domain, proxy on

In Railway, add both custom domains to the service.

---

## Verification Checklist

- [ ] `dampconcrete.com` shows holding page with duck logo and product links
- [ ] `associations.dampconcrete.com` shows full marketing page
- [ ] Download button links to correct S3 DMG URL
- [ ] Both pages render correctly on mobile
- [ ] Cloudflare analytics token loads without errors
- [ ] No console errors
