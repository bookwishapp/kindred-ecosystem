...
class KindredGrid extends StatelessWidget {
  ...
  const KindredGrid({super.key, required this.kin, required this.onAvatarTap});
          ...
          child: SizedBox(
            ...
            child: Stack(
              children: List.generate(kin.length, (index) {
                ...
                final provider = Provider.of<KinProvider>(
                  context,
                  listen: true,
                );
                double verticalProgress = provider.getDisplayPosition(
                  person.id,
                );

                // Calculate the top position for this avatar
                final topPosition =
                    verticalPadding + (verticalProgress * usableHeight);
                ...
                final horizontalPosition =
                    0.15 + (random.nextDouble() * 0.7); // Range: 0.15 to 0.85

                // Calculate left position based on percentage of screen width
                final leftPosition =
                    (availableWidth * horizontalPosition) - (avatarSize / 2);

                return Positioned(
                  top:
                      topPosition -
                      (avatarSize / 2), // Center avatar vertically in its zone
                  left: leftPosition.clamp(
                    horizontalPadding,
                    availableWidth - horizontalPadding - avatarSize,
                  ),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onVerticalDragUpdate: (details) {
        ...
        final dragDelta =
            (details.globalPosition.dy - _dragStartY) / screenHeight;
